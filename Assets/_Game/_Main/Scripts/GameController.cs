...
namespace Main
{
    internal class GameController : MonoBehaviour, IGameState
    {
        ...
        private IEnemyPool enemyFactory;
        ...
        private void InitNextLevel()
        {
            ...
            ReturnPooledEntities();
            levelIndex++;
            enemyFactory = levelIndex % 2 == 0 ? new EnemyFactoryEven() : new EnemyFactoryOdd();
        ...
        private void InstantiateEnemies()
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                IEntity enemy = enemyFactory.GetRandomEnemy(position);
                entities.Add(enemy);
            }
        }
        ...
        private void ReturnPooledEntities()
        {
            if (enemyFactory != null)
            {
                foreach (IEntity e in entities)
                {
                    if (e is IEnemy enemy)
                    {
                        enemyFactory.ReturnToPool(enemy);
                    }
                }
                entities.Clear();
            }
