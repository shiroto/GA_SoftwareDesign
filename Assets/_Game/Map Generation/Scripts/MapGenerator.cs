...
namespace MapGen
{
    ...
    public static class MapGenerator
    {
        ...
        public static IMap GenMap(MapGenConfig c)
        {
            ...
            Walls(map);
            Connect(c, map);
            PlaceStaircase(c, map);
            return map;
        }

        private static void Connect(MapGenConfig mapConfig, Map map)
        {
            ...
            // random start
            Vector2Int startPos = GetRandom(u, mapConfig);
            Mark(map, r, u, startPos);
            // make random connections
            while (u.Count > 0)
            {
                Vector2Int end = GetRandom(r, mapConfig);
                startPos = GetRandom(u, mapConfig);
                ...
                while (current != end)
                {
                    ...
                    nextTile = current + GetDirection(mapConfig, map, wasW, current, ops);
                    Tiles type = map.GetTile(nextTile);
                    if (type == Tiles.NONE || type == Tiles.WALL_HOR || type == Tiles.WALL_VER)
                    {
                        map.SetTile(nextTile, Tiles.PATH);
                        u.Add(nextTile);
                    }
        ...
        private static void GenerateRooms(MapGenConfig config, Map map)
        {
            ...
            {
                int roomWidth = config.Random.Next(config.MinRoomSize, config.MaxRoomSize + 1); int roomHeight = config.Random.Next(config.MinRoomSize, config.MaxRoomSize + 1); int roomX = config.Random.Next(1, config.MapWidth - roomWidth - 1); int roomY = config.Random.Next(1, config.MapHeight - roomHeight - 1);
        ...
        private static Vector2Int GetDirection(MapGenConfig c, Map m, bool wasW, Vector2Int cP, Vector2Int[] v)
        {
            ...
            {
                if
                       (m.GetTile(cP + v[(i + randomOffset) % v.Length]) == Tiles.WALL_VER || m.GetTile(cP + v[(i + randomOffset) % v.Length]) == Tiles.WALL_HOR)
                {
                    continue;
                }
